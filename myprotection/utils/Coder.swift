//
//  Coder.swift
//  myprotection
//
//  Created by Rubeg NPO on 16.11.2021.
//  Copyright © 2021 Rubeg NPO. All rights reserved.
//

import Foundation

typealias Byte = UInt8

class Coder {

    private let codingData: [Byte] = [
        
    ]

    func encode(_ data: [Byte]) -> String {
        let keys = [
            Byte.random(in: Byte.min...Byte.max),
            Byte.random(in: Byte.min...Byte.max)
        ]

        var bytes = keys + data
        code(&bytes)

        return Data(bytes).base64EncodedString()
    }

    func decode(_ base64String: String) -> [Byte]? {
        guard let data = Data(base64Encoded: base64String) else {
            return nil
        }

        var bytes = [Byte](data)
        code(&bytes)

        bytes.remove(at: 0)
        bytes.remove(at: 0)

        return bytes
    }

    private func code(_ arr: inout [Byte]) {
        var left = Int(arr[0])
        var right = Int(arr[1])

        for index in 2..<arr.count {
            arr[index] ^= codingData[left] ^ codingData[right]

            left = left < 0xFF ? left + 1 : 0
            right = right > 0 ? right - 1 : 0xFF
        }
    }

}
